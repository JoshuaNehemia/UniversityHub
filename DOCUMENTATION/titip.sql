...
CREATE SCHEMA `fullstack` DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci ;
-- VISIBLE ku hapus soalnya kalau pakai XAMPP itu pakai MariaDB sedangkan VISIBLE itu cuma bisa MySQL 8++
...
CREATE TABLE IF NOT EXISTS `fullstack`.`akun` (
  ...
  INDEX `fk_akun_mahasiswa_idx` (`nrp_mahasiswa` ASC) ,
  INDEX `fk_akun_dosen1_idx` (`npk_dosen` ASC) ,
...
CREATE TABLE IF NOT EXISTS `fullstack`.`grup` (
  ...
  PRIMARY KEY (`idgrup`),
  INDEX `fk_grup_akun1_idx` (`username_pembuat` ASC) ,
...
CREATE TABLE IF NOT EXISTS `fullstack`.`thread` (
  ...
  INDEX `fk_thread_akun1_idx` (`username_pembuat` ASC) ,
  INDEX `fk_thread_grup1_idx` (`idgrup` ASC) ,
...
CREATE TABLE IF NOT EXISTS `fullstack`.`chat` (
  ...
  `tanggal_pembuatan` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`idchat`),
  INDEX `fk_chat_thread1_idx` (`idthread` ASC) ,
  INDEX `fk_chat_akun1_idx` (`username_pembuat` ASC) ,
...
CREATE TABLE IF NOT EXISTS `fullstack`.`member_grup` (
  ...
  INDEX `fk_grup_has_akun_akun1_idx` (`username` ASC) ,
  INDEX `fk_grup_has_akun_grup1_idx` (`idgrup` ASC) ,
...
CREATE TABLE IF NOT EXISTS `fullstack`.`event` (
  ...
  PRIMARY KEY (`idevent`),
  INDEX `fk_event_grup1_idx` (`idgrup` ASC) ,
